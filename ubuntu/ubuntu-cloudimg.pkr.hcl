...
build {
  ...
  provisioner "file" {
    destination = "/tmp/"
    sources     = ["${path.root}/scripts/examples/openvswitch-switch-dummy/openvswitch-switch_9999_all.deb"]
  }

  provisioner "shell" {
    environment_vars  = concat(local.proxy_env, ["DEBIAN_FRONTEND=noninteractive"])
    expect_disconnect = true
    scripts           = ["${path.root}/scripts/examples/openvswitch-switch-dummy.sh"]
  }
